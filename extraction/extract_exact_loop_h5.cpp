...
void usage(char exe[]){
  printf("%s: <.h5-file> <output_file> <loop (Scalar, dOp, Loops, LoopsCv, LpsDw, LpsDwCv)> <momx> <momy> <momz> <conf_traj> <T>\n",exe);
  exit(-1);
}
...
int main(int argc, char *argv[]){
  ...
  if(loop_oneD[dt]){
    for(int mu=0;mu<DIR;mu++){
      asprintf(&group_dir,"/conf_%s/%s/mom_xyz_%s_%s_%s/dir_%02d",conf,loop,mom[0],mom[1],mom[2],mu);
    ...
    asprintf(&group_dir,"/conf_%s/%s/mom_xyz_%s_%s_%s",conf,loop,mom[0],mom[1],mom[2]);
    group_id = H5Gopen(file_id, group_dir, H5P_DEFAULT);    
    dset_id = H5Dopen(group_id, "loop", H5P_DEFAULT);
