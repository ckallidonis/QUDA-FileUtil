...
void usage(char exe[]){
  printf("%s: <.h5-file prefix> <threep ASCII in_dir> <mom_list> <tsink_list> <src_list> <Nmoms> <Qsq> <Nsrc> <T> <conf-trajectory>\n",exe);
  exit(-1);
}
//=============================================================
...
int main(int argc, char *argv[]){
  ...
  char *h5_file,*in_dir,*mom_list,*tsink_list,*src_list;
  asprintf(&h5_file   ,"%s",argv[1]);
  asprintf(&in_dir    ,"%s",argv[2]);
  asprintf(&mom_list  ,"%s",argv[3]);
  asprintf(&tsink_list,"%s",argv[4]);
  asprintf(&src_list  ,"%s",argv[5]);
  int Nmoms  = atoi(argv[6]);
  int Qsq    = atoi(argv[7]);
  int Nsrc   = atoi(argv[8]);
  int T      = atoi(argv[9]);
  ...
  for(int is=0;is<Nsrc;is++){
	...
	for(int uORd=0;uORd<2;uORd++){ // up and down
	  asprintf(&ASCII_file,"%s/threep.%04d_tsink%d.neutron.%s.%s.SS.%02d.%02d.%02d.%02d.dat",in_dir,conf,tsink[its],thrp_part[uORd],thrp_type[thrp_int],src_pos[is][0],src_pos[is][1],src_pos[is][2],src_pos[is][3]);
    ...
    for(int i=0;i<4;i++) thrpInfo.src_pos[i] = src_pos[is][i];
