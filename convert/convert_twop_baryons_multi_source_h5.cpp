/* Christos Kallidonis                                     */
/* July 2016                                               */
/* This program reads baryon two-point functions written   */
...
void usage(char exe[]){
  printf("%s: <.h5-file prefix> <twop ASCII in_dir> <mom_list> <src_list> <Nmoms> <Qsq> <Nsrc> <T> <conf-trajectory> <baryon: all, nucl>\n",exe);
  exit(-1);
}
//=============================================================

void writeTwop_HDF5(Float *twopBaryons, char *h5_file, int **mom, Info info){
  ...
  char fname[512];
  sprintf(fname,"%s.%04d_baryons_Qsq%d_SS.%02d.%02d.%02d.%02d.h5",h5_file,info.conf,info.Qsq,info.src_pos[0],info.src_pos[1],info.src_pos[2],info.src_pos[3]);
  ...
  for(int bar=0;bar<info.N_PART;bar++){
	...
	writeTwopBuf = &(twopBaryons[2*16*T*imom + 2*16*T*Nmoms*bar + 2*16*T*Nmoms*info.N_PART*ip]);
...
int main(int argc, char *argv[]){
  ...
  char *h5_file,*in_dir,*mom_list,*src_list, *run_type;
  ...
  asprintf(&run_type,"%s",argv[10]);

  int N_BARYONS;
  if(strcmp(run_type,"all")==0) N_BARYONS=10;
  else if(strcmp(run_type,"nucl")==0) N_BARYONS=1;
  else errorMsg("Wrong baryon type given. Must be either 'all' or 'nucl'\n");
  ...
  printf("T      = %d\n",T);
  printf("conf   = %d\n",conf);
  printf("Will run for %s\n",run_type);
  //-------------------------------------

  //-Allocate the write buffer
  Float *Twop_HDF5 = NULL;

  if( (Twop_HDF5 = (Float*) malloc(2*16*T*Nmoms*2*N_BARYONS*sizeof(Float)))==NULL ) errorMsg("Cannot allocate memory for Twop_HDF5.\n");
  //-------------------------------------
  ...
  for(int is=0;is<Nsrc;is++){
    asprintf(&ASCII_file,"%s/twop.%04d.baryons.SS.%02d.%02d.%02d.%02d.dat",in_dir,conf,src_pos[is][0],src_pos[is][1],src_pos[is][2],src_pos[is][3]);
	...
	for(int imom=0;imom<Nmoms;imom++){
	  for(int im=0;im<16;im++){
	    fscanf(p_twop,"%d %d %d %d %d %d %d %f %f %f %f\n",&idum,&idum,&idum,&idum,&idum,&idum,&idum,
		   &(Twop_HDF5[0 + 2*im + 2*16*it + 2*16*T*imom + 2*16*T*Nmoms*bar + 2*16*T*Nmoms*N_BARYONS*0]), &(Twop_HDF5[1 + 2*im + 2*16*it + 2*16*T*imom + 2*16*T*Nmoms*bar + 2*16*T*Nmoms*N_BARYONS*0]),
		   &(Twop_HDF5[0 + 2*im + 2*16*it + 2*16*T*imom + 2*16*T*Nmoms*bar + 2*16*T*Nmoms*N_BARYONS*1]), &(Twop_HDF5[1 + 2*im + 2*16*it + 2*16*T*imom + 2*16*T*Nmoms*bar + 2*16*T*Nmoms*N_BARYONS*1]));
	  }}}
    }
    fclose(p_twop);
    ...
    twopInfo.Nmoms = Nmoms; twopInfo.Qsq = Qsq; twopInfo.T = T; twopInfo.conf = conf; twopInfo.N_PART = N_BARYONS;
    for(int i=0;i<4;i++) twopInfo.src_pos[i] = src_pos[is][i];
    for(int i=0;i<N_BARYONS;i++) strcpy(twopInfo.twop_type[i],twop_type[i]);
