...
int main(int argc, char *argv[]){
  ...
  char *file;
  if(h5out){
    asprintf(&file,"%s.%s_%s_Qsq%d_SS.%02d.%02d.%02d.%02d.h5",outpre, conf, READ_ALL ? "mesons" : twop_type[dt], Qsq, src[0], src[1], src[2], src[3]);
    writeTwop_HDF5(twopMom, file, mom, twopInfo);
  }
  else{
    asprintf(&file,"%s.%s.%s.SS.%02d.%02d.%02d.%02d.dat",outpre, conf, READ_ALL ? "mesons" : twop_type[dt], src[0], src[1], src[2], src[3]);
    printf("The output ASCII file is: %s\n",file);
