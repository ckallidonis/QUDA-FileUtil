...
int main(int argc, char *argv[]){
  ...
  float *twopBuf = (float*) malloc(Np*T*Nmoms*M*2*sizeof(float));
  ...
  for(int ip=Ns;ip<(Np+Ns);ip++){
    ...
    herr_t status = H5Dread(dset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(twopBuf[(ip%Np)*T*Nmoms*M*2]));
  ...
  if(xtype==0){
	...
	for(int ga=0;ga<4;ga++){
	  for(int gap=0;gap<4;gap++){
	    int m = gap + 4*ga;
	    fprintf(out,"%d\t%02d\t%+d %+d %+d\t%d %d\t%+e %+e\t%+e %+e\n",dt,t,moms[0+3*imom],moms[1+3*imom],moms[2+3*imom],ga,gap,
		    twopBuf[0 + 2*m + 2*M*imom + 2*M*Nmoms*t + 2*M*Nmoms*T*0],twopBuf[1 + 2*m + 2*M*imom + 2*M*Nmoms*t + 2*M*Nmoms*T*0],
		    twopBuf[0 + 2*m + 2*M*imom + 2*M*Nmoms*t + 2*M*Nmoms*T*1],twopBuf[1 + 2*m + 2*M*imom + 2*M*Nmoms*t + 2*M*Nmoms*T*1]);
	  }}}
    }//-imom
  }
  else{
	...
	for(int ga=0;ga<4;ga++){
	  for(int gap=0;gap<4;gap++){
	    int m = gap + 4*ga;
	    fprintf(out,"%d\t%02d\t%+d %+d %+d\t%d %d\t%+e %+e\n",dt,t,moms[0+3*imom],moms[1+3*imom],moms[2+3*imom],ga,gap,
		    twopBuf[0 + 2*m + 2*M*imom + 2*M*Nmoms*t],twopBuf[1 + 2*m + 2*M*imom + 2*M*Nmoms*t]);
	  }}}
    }//-imom
  }
