...
void usage(char exe[]){
  printf("%s: <.h5-file> <output_file> <loop (Scalar, dOp, Loops, LoopsCv, LpsDw, LpsDwCv)> <conf_traj> <Nvec> <HighPrec:0, LowPrec:1> <T>\n",exe);
  exit(-1);
}
...
int main(int argc, char *argv[]){
  ...
  int Ns = atoi(argv[5]);
  bool LowPrecSum = (atoi(argv[6]) == 1 ? true : false);
  int T = atoi(argv[7]);
  ...
  if(loop_oneD[dt]){
    for(int mu=0;mu<DIR;mu++){
      ...
      asprintf(&group_dir,"/conf_%s/%s_%04d/%s/dir_%02d",conf,vec_str,Ns,loop,mu);
    ...
    asprintf(&group_dir,"/conf_%s/%s_%04d/%s",conf,vec_str,Ns,loop);
    group_id = H5Gopen(file_id, group_dir, H5P_DEFAULT);    
    dset_id = H5Dopen(group_id, "loop", H5P_DEFAULT);
